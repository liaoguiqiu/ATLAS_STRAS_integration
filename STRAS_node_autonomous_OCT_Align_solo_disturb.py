...
class node():
  ...
  def callback_oct_dis(self,data):
    ...
    if time_stamp > self.move_time:
      ...
      if (abs( self.dy  ) > 30):
        new_gain =self.gain*6 * abs( self.dy   )/100
      else:
        
        new_gain =self.gain*4
      new_gain =self.gain*0.8
      self.dy =  np.clip(self.dy,-100,100)
      self.dy_integral = self.dy_integral  + 0.0*self.dy
      self.dy_integral =  np.clip(self.dy_integral,-100,100)
      #targety =     kine_state[6]  -new_gain*(self.dy + 0.1* (self.dy-self.dy_l)) # PD with differential
      deltay = -(new_gain*self.dy + 0*self.dy_integral  + self.gain*300* (self.dy-self.dy_l[len(self.dy_l)-1]))
      deltay = np.clip(deltay,-1,1)
      print("DELTA y =  " + str(deltay))
      #targety =     np.mean(self.state_filter) + deltay # PD with differential
      targety =  kine_state[6] + deltay
      targety = np.clip(targety,-1,5)
      print("STATE y =  " + str(np.mean(self.state_filter) ))
      print("target y =  " + str(targety))
      #speed = 0.09
      speed = 0.1*abs( self.dy  )/30 + 0.09# The max speed is around 0.09
      ...
      if  self.state_flag:
        ...
        outputCommand.append(['EBH', kine_state_updated[1], 0.02 ])

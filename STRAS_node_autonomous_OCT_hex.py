...
class node():
  def __init__(self):
    ...
    host = '192.168.12.103'
    port_slave = 6666
    self.STRAS_slave = STRASslave(host=host, port=port_slave)
  ...
  def callback_oct_dis(self,data):
    ...
    if time_stamp > self.move_time:
      ...
      print("endoscope rotation lication ")
      print(str(kine_state[2]))
      # targetx = kine_state[1] - self.gain*self.dx
      # targety = kine_state[0] - self.gain*self.dy
      new_gain =self.gain*2* abs(self.dy) /30
      targety =     kine_state[9]  -new_gain*(self.dy + 0.1* (self.dy-self.dy_l)) # PD with differential
      speed = abs(self.dy + 0.1* (self.dy-self.dy_l))*0.15 /20 # The max speed is around 0.09
      self.dy_l  = self.dy
      ...
      if self.trans_stage ==1:
          ...
          kine_state_updated[1] = self.ER_p1
          if ((abs(kine_state[7]-self.trans_P1)<2) and (abs(kine_state[1]-self.ER_p1)<2)):
            self.trans_stage =2
      else:
          targetz = self.trans_P2  #  for the OCT tempoarily sidaable the trans;ation 
          kine_state_updated[1] = self.ER_p2
          if  ((abs(kine_state[7]-self.trans_P2)<2) and (abs(kine_state[1]-self.ER_p2)<2)):
            self.trans_stage =1
      self.current_arm_tran = kine_state[7] # record the current translation state 
      ...
      if  self.state_flag:
        ...
        outputCommand.append(['BL', kine_state_updated[9], speed ])

...
class node():
  ...
  def callback_oct_dis(self,data):
    ...
    if (self.current_oct_contact > 110  ):
      Over_press = 1.0
    else:
      Over_press = 0
    if (self.current_oct_dis>30 or self.current_oct_contact<5):
      lose_sight = 2
    else:
      lose_sight = 0
    alfa= 0.9
    self.target_y = self.target_y  + (1-alfa)*(Over_press)
    self.target_y = self.target_y -(1-alfa)*(lose_sight)
    self.target_y = -30
    target_y =  self.target_y 
    print("INI target6y =  " + str(target_y))
    self.dy =   -(self.current_oct_dis  - target_y - 1.0*self.current_oct_contact )  # this is horizontal 
    self.gain =0.01  # between 500-2000
    #self.dy = 0.7*self.dy_l[len(self.dy_l)-1] +0.3*self.dy
    # self.dy = 0 
    self.dz = 0.4
    ...
    if time_stamp > self.move_time:
      ...
      self.state_filter=np.append(self.state_filter, [kine_state[6]] ,axis=0)
      self.state_filter= np.delete(self.state_filter , 0,axis=0)
      # in mm 
      self.dy = self.dy*5/256   

      arm_length  = 20.0
      cable_unit =  4.5 
      # in mm
      arc_control = arm_length/cable_unit
      self.gain = 1.0/ arc_control
      kd = 0.5
      if ( self.current_oct_dis  < 20 ):
        new_gain =self.gain *0.9
      else:
        
        new_gain =self.gain 

      if ( self.dy  < 0 ):
        new_gain =new_gain *1.5
      else:
      ...
      if ( self.current_oct_dis  < 100 ):
        ...
        kd =0.0
      # new_gain =self.gain*0.3
      self.dy =  np.clip(self.dy,-5,5)
      self.dy_integral = self.dy_integral  + 0.0*self.dy
      self.dy_integral =  np.clip(self.dy_integral,-5,5)


      
      #targety =     kine_state[6]  -new_gain*(self.dy + 0.1* (self.dy-self.dy_l)) # PD with differential
      deltay = (new_gain*self.dy + 0*self.dy_integral  + new_gain*kd* (self.dy-self.dy_l[len(self.dy_l)-2]))
      deltay = np.clip(deltay,-2,2)
      print("DELTA y =  " + str(deltay))
      #targety =     np.mean(self.state_filter) + deltay # PD with differential
      # targety =  kine_state[6] + deltay
      targetEB =  kine_state[1] + deltay 

      targetEB = np.clip(targetEB,-4,0)
      print("STATE y =  " + str(np.mean(self.state_filter) ))
      print("target y =  " + str(targetEB))
      #speed = 0.09
      speed = abs( deltay  )  # The max speed is around 0.09
      speed = np.clip(speed,-2,2)
      self.state_filter
      self.dy_l=np.append(self.dy_l, [self.dy] ,axis=0)
      self.dy_l= np.delete(self.dy_l , 0,axis=0)
      ...
      targety=kine_state_updated[6]
      if (targetEB < -3):
        bendingarm  =  1 
      else:
        bendingarm  =  0
      targety = kine_state_updated[6] + 0.5*(bendingarm-kine_state_updated[6])
      ######################################""
      if self.trans_stage ==1:
          targetz = self.trans_P1  #  for the OCT tempoarily sidaable the trans;ation 
          if (abs(kine_state[4]-self.trans_P1)<1):
            self.trans_stage =2
      else:
          targetz = self.trans_P2  #  for the OCT tempoarily sidaable the trans;ation 
          if (abs(kine_state[4]-self.trans_P2)<1):
            self.trans_stage =1
      ...
      # if self.distrub_stage ==1:
      #     targetEB = self.disturb_P1  #  for the OCT tempoarily sidaable the trans;ation 
      #     if (abs(kine_state[1]-self.disturb_P1)<0.5):
      #       self.distrub_stage =2
      # else:
      #     targetEB = self.disturb_P2   #  for the OCT tempoarily sidaable the trans;ation 
      #     if (abs(kine_state[1]-self.disturb_P2)<0.5):
      #       self.distrub_stage =1 
      # ##############################################""
      # targetEB = 4.5
      ######################################""
      #self.current_arm_tran = kine_state[4] # record the current translation state 
      ...
      if  self.state_flag:
        ...
        outputCommand.append(['EBH', kine_state_updated[1], speed ])
        outputCommand.append(['TR', kine_state_updated[4], 0.02 ])
